...
public class AluguelDAO {
    ...
    public static List<Aluguel> getAluguel() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                listaAluguel.add(new Aluguel(id, dataLocacao, dataDevolucao, juros, localRetirada, localDevolucao, qntDiaAluguel, idCliente, idCarro));
    ...
    //VERIFICAR ONDE ESTA SENDO USADO ESSE METODO
    public static Aluguel getAluguel(int id) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                int idCliente = rs.getInt("idCliente");
                int idCarro = rs.getInt("idCarro");
                //aluguel = new Aluguel(dataLocacao, dataDevolucao, localRetirada, localDevolucao, qntDiaAluguel, idCliente, idCarro);
    ...
    public static void updateAluguel(Aluguel aluguel) throws SQLException, ClassNotFoundException {
        Connection con = ConexaoBD.getConexao();
        String query = ("update aluguel set dataLocacao= ?, dataDevolucao= ?, juros= ?, localRetirada= ?, localDevolucao= ?, qntDiaAluguel= ?, idCliente= ?, idCarro= ? where id= ?");
        PreparedStatement ps = con.prepareStatement(query);
        ps.setString(1, aluguel.getDataLocacao());
        ps.setString(2, aluguel.getDataDevolucao());
        ps.setDouble(3, aluguel.getJuros());
